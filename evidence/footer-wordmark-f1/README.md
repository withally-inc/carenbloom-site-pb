...
- **Motion.** The reference loops continuously as a marquee. Ours is a one-shot arrival in the homepage's unhurried transform-and-opacity character (the top-level README owns the arrival paths), and it never animates again. Perpetual motion in a page footer is a distraction and an accessibility cost we are not willing to pay.
