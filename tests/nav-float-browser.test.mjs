...
try {
  ...
  assert.equal(await desktop.locator(".topbar .roles-chip").textContent(), "Open roles (15)");
  ...
  for (const width of [360, 320]) {
    ...
    assert.equal(await narrow.locator(".topbar .roles-chip").textContent(), "Open roles (15)");
    await narrow.close();
  }
