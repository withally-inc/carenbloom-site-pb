...
- `scripts/careers-apply.js` and `scripts/careers-home.js` render only non-cacheable `/api/role-state` responses; static, unknown, closed, and endpoint-failure surfaces stay fail-closed, and `tests/role-state-{api,browser}.test.mjs` pin that contract.
