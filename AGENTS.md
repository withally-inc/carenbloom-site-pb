...
- The footer sign-off is live Syne text owned by `[data-footer-wordmark]` in `index.html`, its progressive one-shot observer in `app.js`, and `tests/footer-wordmark.test.mjs`; default, reduced-motion, and no-JavaScript states must remain fully visible, and `evidence/footer-wordmark-f1/README.md` records the deliberate divergences from the Mobbin reference.
