...
- `scripts/careers-apply.js` and `scripts/careers-home.js` render only non-cacheable role-state responses, requested through the single `scripts/role-state-endpoint.js` resolver that owns the production API origin; static, unknown, closed, and endpoint-failure surfaces stay fail-closed, and `tests/role-state-{api,browser,endpoint}.test.mjs` pin that contract.
